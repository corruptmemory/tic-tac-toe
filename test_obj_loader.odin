...
@private
wos_advance_pos :: #force_inline proc(scanner: ^Wavefront_Object_Scanner, amount: u32 = 1) -> bool {
...
@private
wos_advance_line :: #force_inline proc(scanner: ^Wavefront_Object_Scanner, amount: u32 = 1) {
  scanner.line += amount;
}


@private
wos_set_vertex_base_index :: #force_inline proc(scanner: ^Wavefront_Object_Scanner, point: u32) {
  scanner.vertex_base = point;
}

@private
wos_set_texture_coords_base_index :: #force_inline proc(scanner: ^Wavefront_Object_Scanner, point: u32) {
  scanner.texture_coords_base = point;
}

@private
wos_set_vertex_normals_base_index :: #force_inline proc(scanner: ^Wavefront_Object_Scanner, point: u32) {
  scanner.vertex_normals_base = point;
}

@private
wos_byte_at_pos :: #force_inline proc(scanner: ^Wavefront_Object_Scanner) -> byte {
  return scanner.bytes[scanner.pos];
}

@private
wos_init :: proc(scanner: ^Wavefront_Object_Scanner, bytes: []byte) {
  scanner.bytes = bytes;
...
@private
wos_finished :: #force_inline proc(scanner: ^Wavefront_Object_Scanner) -> bool {
  return scanner.pos == scanner.eos;
}

@private
eos_fixup :: #force_inline proc(scanner: ^Wavefront_Object_Scanner) {
  if scanner.pos > scanner.eos do scanner.pos = scanner.eos;
}


@private
past_eol :: proc(scanner: ^Wavefront_Object_Scanner) -> bool {
 ...
 */
@private
consume_to_eol :: proc(scanner: ^Wavefront_Object_Scanner) -> []byte {
...
@private
consume_number :: proc(scanner: ^Wavefront_Object_Scanner) -> []byte {
  start, end: u32 = scanner.pos, scanner.pos;
  stop:
  for !wos_finished(scanner) {
...
@private
consume_f32 :: proc(scanner: ^Wavefront_Object_Scanner) -> (f32, bool) {
...
@private
consume_u32 :: proc(scanner: ^Wavefront_Object_Scanner) -> (u32, bool) {
  bs := consume_number(scanner);
  if len(bs) > 0 {
...
@private
past_whitespace :: proc(scanner: ^Wavefront_Object_Scanner) {
  for !wos_finished(scanner) {
    switch wos_byte_at_pos(scanner) {
      case ' ', '\t':
...
@private
keyword_check :: #force_inline proc(scanner: ^Wavefront_Object_Scanner, keyword: string) -> bool {
...
obj_obj_parser :: proc(scanner: ^Wavefront_Object_Scanner, allocator := context.allocator) -> (^Wavefront_Object, bool) {
...
obj_parser :: proc(object_file: ^Wavefront_Object_File, bytes: []byte, allocator := context.allocator) -> bool {
      ...
      case 'o':
        if keyword_check(&scanner, O_KEYWORD) {
          ...
          if len(o) > 0 {
            ...
            obj, ok := obj_obj_parser(&scanner,allocator);
            if ok {
...
init_object_file :: proc(object: ^Wavefront_Object_File) {
  object.mtllibs = make([dynamic]string);
  object.objects = make([dynamic]^Wavefront_Object);
}

obj_loader :: proc(file: string, object: ^Wavefront_Object_File, allocator := context.allocator) -> bool {
...
destroy_object_file :: proc(object: ^Wavefront_Object_File) {
