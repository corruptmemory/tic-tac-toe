...
init :: proc(ctx: ^Context) {
  ...
  tsync.channel_init(&ctx.xcbChannel, 10);
  tsync.channel_init(&ctx.getInputChannel, 1);
...
get_input_state :: proc(ctx: ^Context, uiState: ^input.UIState) -> bool {
  ...
  for _, ok := tsync.channel_try_recv(&done); !ok; _, ok = tsync.channel_try_recv(&done) {
